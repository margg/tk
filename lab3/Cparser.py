...
class Cparser(object):
    ...
    def p_program(self, p):
        ...
        p[0] = AST.Program(p[1]).add_lineno(p.lineno)
    ...
    def p_declaration(self, p):
        """declaration : TYPE inits ';'
                       | error ';' """
        if len(p) == 4:
            p[0] = AST.Declaration(AST.Name(p[1]).add_lineno(p.lineno), p[2]).add_lineno(p.lineno)
    ...
    def p_init(self, p):
        """init : ID '=' expression """
        p[0] = AST.Initializer(AST.Name(p[1]).add_lineno(p.lineno), p[3]).add_lineno(p.lineno)
    ...
    def p_print_instr(self, p):
        """print_instr : PRINT expr_list ';'
                       | PRINT error ';' """
        p[0] = AST.PrintInstr(p[2]).add_lineno(p.lineno)

    def p_labeled_instr(self, p):
        """labeled_instr : ID ':' instruction """
        p[0] = AST.LabeledInstr(AST.Name(p[1]).add_lineno(p.lineno), p[3]).add_lineno(p.lineno)

    def p_assignment(self, p):
        """assignment : ID '=' expression ';' """
        p[0] = AST.Assignment(AST.Name(p[1]).add_lineno(p.lineno), p[3]).add_lineno(p.lineno)
    ...
    def p_choice_instr(self, p):
        ...
        p[0] = AST.IfInstr(p[3], p[5], None if len(p) < 8 else p[7]).add_lineno(p.lineno)

    def p_while_instr(self, p):
        """while_instr : WHILE '(' condition ')' instruction
                       | WHILE '(' error ')' instruction """
        p[0] = AST.WhileInstr(p[3], p[5]).add_lineno(p.lineno)

    def p_repeat_instr(self, p):
        """repeat_instr : REPEAT instructions UNTIL condition ';' """
        p[0] = AST.RepeatInstr(p[2], p[4]).add_lineno(p.lineno)

    def p_return_instr(self, p):
        """return_instr : RETURN expression ';' """
        p[0] = AST.ReturnInstr(p[2]).add_lineno(p.lineno)

    def p_continue_instr(self, p):
        """continue_instr : CONTINUE ';' """
        p[0] = AST.ContinueInstr().add_lineno(p.lineno)

    def p_break_instr(self, p):
        """break_instr : BREAK ';' """
        p[0] = AST.BreakInstr().add_lineno(p.lineno)
    ...
    def p_fundef(self, p):
        ...
        p[0] = AST.FunctionDef(AST.Name(p[1]).add_lineno(p.lineno),
                               AST.Name(p[2]).add_lineno(p.lineno), p[4], p[6]).add_lineno(p.lineno)
    ...
    def p_arg(self, p):
        """arg : TYPE ID """
        p[0] = AST.Argument(AST.Name(p[1]).add_lineno(p.lineno),
                            AST.Name(p[2]).add_lineno(p.lineno)).add_lineno(p.lineno)
